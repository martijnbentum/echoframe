...
def find_embedding_layers(store, phraser_key, collar, model_name, layers):
...
def segment_times(segment, collar):
    ...
    collar = ms_to_seconds(collar)
    start_seconds = float(segment.start_seconds)
    end_seconds = float(segment.end_seconds)
    duration_seconds = float(ms_to_seconds(segment.audio.duration))
...
def validate_frame_aggregation(frame_aggregation):
...
def validate_codebook_artifacts(artifacts):
